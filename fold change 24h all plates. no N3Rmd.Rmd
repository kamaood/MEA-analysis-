---
title: "fold change 24h all parameters"
...
Path_to_working_directory <- "/Users/karaodriscoll/Documents/Lab Masters/fold change 24h"
...
looking at raw data first before fold change
...
raw_data.1 <- subset(raw_data, raw_data$`Number of Spikes` != 0) #removing NA and values of 0
...
ggplot(raw_data.1, aes(x = `Treatment`, y = `fold_change_spikes`, colour = `Treatment`, fill = `Treatment`))+
 ...
 geom_boxplot(aes(x = `Treatment`, y = `fold_change_spikes`, fill = `Treatment`), outlier.shape = NA, alpha = .5, width = .1, colour = "black") +
  ...
  guides(fill = FALSE)+
  ggtitle('Number of Spikes 24h treated')

ggsave("raw_data.1.png")
```
...
```{r}
raw_data.mf <- subset(raw_data, raw_data$`Mean Firing Rate (Hz)` != 0) #removing NA and values of 0
data_for_stats.2 <- raw_data.mf %>% 
  drop_na(`Mean Firing Rate (Hz)`) %>%
  mutate("LogParameter"=log(`Mean Firing Rate (Hz)`))

model_treatment.2 <- lm(`LogParameter` ~ Treatment * Time, data = data_for_stats.2) 
 #diagnostic plots
...
ggplot(raw_data, aes(x = `Treatment`, y = `fold_change_mfr`, colour = `Treatment`, fill = `Treatment`))+
 ...
 geom_boxplot(aes(x = `Treatment`, y = `fold_change_mfr`, fill = `Treatment`), outlier.shape = NA, alpha = .5, width = .1, colour = "black") +
  ...
  guides(fill = FALSE)+
  ggtitle('Mean Firing Rate 24h treated')

ggsave("raw_data.mf.png")
```
...
```{r}
raw_data.spb <- subset(raw_data, raw_data$`Number of Spikes per Burst - Avg` != 0) #removing NA and values of 0
data_for_stats.5 <- raw_data.spb %>% 
...
not data from every plate/ only one well really per condition per plate / even less sometimes - only one well N2 doi
...
ggplot(spb_data, aes(x = `Treatment`, y = `fold_change_spb`, colour = `Treatment`, fill = `Treatment`))+
 ...
 geom_boxplot(aes(x = `Treatment`, y = `fold_change_spb`, fill = `Treatment`), outlier.shape = NA, alpha = .5, width = .1, colour = "black") +
  ...
  theme_cowplot()+
  guides(fill = FALSE)+
  ggtitle('Spikes per Burst 24h treated')
...
```{r}
raw_data.bf <- subset(raw_data, raw_data$`Burst Frequency - Avg (Hz)` != 0) #removing NA and values of 0
data_for_stats.7 <- raw_data.bf %>% 
...
plate and treatment x plate sig
...
ggplot(bf_data, aes(x = `Treatment`, y = `fold_change_bursts`, colour = `Treatment`, fill = `Treatment`))+
 ...
 geom_boxplot(aes(x = `Treatment`, y = `fold_change_bursts`, fill = `Treatment`), outlier.shape = NA, alpha = .5, width = .1, colour = "black") +
  ...
  theme_cowplot()+
  guides(fill = FALSE)+
  ggtitle('Burst Frequency 24h treated')
...
```{r}
raw_data.si <- subset(raw_data, raw_data$`Synchrony Index` != 0) #removing NA and values of 0
data_for_stats.8 <- raw_data.si %>% 
...
ggplot(si_data, aes(x = `Treatment`, y = `fold_change_si`, colour = `Treatment`, fill = `Treatment`))+
 ...
 geom_boxplot(aes(x = `Treatment`, y = `fold_change_si`, fill = `Treatment`), outlier.shape = NA, alpha = .5, width = .1, colour = "black") +
  ...
  theme_cowplot()+
  guides(fill = FALSE)+
  ggtitle('Synchrony Index 24h treated')
```
...
N1- 17, N2 - 21 active wells - so inc? with days, N3 is 10 active wells so decreases
